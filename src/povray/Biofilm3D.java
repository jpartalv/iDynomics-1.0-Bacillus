...
public class Biofilm3D implements Serializable {
	...
	protected void modelStateToFile(FileWriter f, String _camera_location ) throws IOException {
		biofilmHeaderToFile(f);
		particlesToFile(f);
		
		String screen_info = "#declare camera_location = " + _camera_location + "\n" +
		...
		"text {ttf \"Arial\", \"Iteration :" +  String.valueOf(SimTimer.getCurrentTime()) + " hrs. , 0.01, 0\n" +
		"    pigment {rgb 1} finish {ambient 1 diffuse 0} no_shadow\n" + 
		"    scale 0.04 translate <.2, -.35, 1> // Adjust position and size\n" +
		"    matrix <-X.x, -X.y, -X.z, Y.x, Y.y, Y.z, Z.x, Z.y, Z.z, 0, 0, 0>\n" +
		"    scale 1/100 translate camera_location}\n" + 
		"text {ttf \"Arial\", \"Time: " + String.valueOf(SimTimer.getCurrentIter()) + ", 0.01, 0\n" +
		"    pigment {rgb 1} finish {ambient 1 diffuse 0} no_shadow\n" +
		"    scale 0.04 translate <.2, -.4, 1> // Adjust position and size\n" +
		"    matrix <-X.x, -X.y, -X.z, Y.x, Y.y, Y.z, Z.x, Z.y, Z.z, 0, 0, 0>\n" +
		"    scale 1/100 translate camera_location}\n";
